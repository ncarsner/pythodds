...
### Fixed
> **Upgrade note:** `anova`, `chisq`, and `linreg` now report different p-values than 0.24.0 did for strongly significant results. The previous values were exactly `0.0`, or carried fewer correct digits than were printed. Any p-value below about 1e-16 read off an earlier run should be recomputed.

- Upper-tail p-values were computed as `1 - cdf` and cancelled to exactly `0.0` in the far tail (#59)
  - `anova`'s `f_sf` returned `0.0` from about F = 412 on (2, 20) df, where the true p-value is 5.6e-17, and lost digits well before that. The reported case — three tight groups centred on 10, 20, and 30 — printed `"p_value": 0.0` where the true value is 5.66e-23. Now evaluated from `I_{df2/(df2 + df1·f)}(df2/2, df1/2)` directly, matching scipy to 1e-13 relative down to 1e-35
  - The studentized range tail behind Tukey HSD returned `0.0` from about q = 34 at k = 4, df = 20, so every pairwise comparison between clearly separated groups reported `0.0`. Now mixes the known-variance range *tail* over the distribution of S/σ instead of subtracting its CDF from 1. Two further changes were needed for that to hold up: Φ is evaluated with `erfc`, since `0.5·(1 + erf(z/√2))` returns 0 for Φ(-27) ≈ 1e-160, and both quadrature windows now follow the mode of the integrand, which moves far from the centre of the old fixed windows once q is large
  - `chisq`'s `chi2_sf` returned `0.0` from about x = 79 on 3 df. Now taken from the upper incomplete gamma directly — the continued fraction already computes Q(a, x), so the tail no longer passes through a subtraction — and matches scipy to 1e-14 relative down to 1e-218
  - `linreg`'s two reported p-values both cancelled: the F-test p-value as `1 - f_cdf`, exactly `0.0` from about F = 291 on (1, 30) df, and the coefficient p-values twice over, since `t_cdf` itself forms `1 - tail`, exactly `0.0` from about t = 16.6 on 30 df. Added `f_sf` and `t_sf` in direct form and moved both callers onto them; `f_cdf` and `t_cdf` are unchanged and still serve `inverse_t_cdf`
  - Added far-tail oracle tests to all three modules. The existing oracle tests compared against scipy at *absolute* tolerance, which any implementation returning `0.0` passes once the true value drops below 1e-16 — which is why none of them caught this. The studentized range is checked against an exact identity instead of scipy: for k = 2 the range is √2·|T|, so its tail is a two-sided t p-value, and scipy's own `studentized_range` integration floors out near 1e-15

- Reported p-values are no longer rounded away by the table formatters. Fixed-point output printed every strongly significant result as `0.0000` — indistinguishable from the collapse above, and it left the CLI showing 0 for a p-value that is now computed correctly. `anova`, `chisq`, and `linreg` switch to scientific notation for any p-value that would round to zero, so the reported case now prints `5.6627e-23` where it printed `0.0000`. JSON output is unchanged and still carries the raw float
- Documented the one floor that does exist. Below the smallest normal double (~2e-308) a tail either underflows to zero or lands among the denormals, where the significand has already lost digits; both are now shown as `<2e-308` rather than as `0.0000`, and each survival function's docstring says that a returned `0.0` means "smaller than double precision can represent", not "impossible". This is a representation limit, not a tolerance: the direct forms carry bounded *relative* error (~1e-13, set by the kernels' `_EPS`), so every digit printed above that floor is real
